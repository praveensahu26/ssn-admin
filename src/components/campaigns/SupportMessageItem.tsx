...
export function SupportMessageItem({ image, name, donatedAmount, words }: SupportMessageItemProps) {
  const [imgError, setImgError] = useState(false);
  const initials = getInitials(name);
  const showImage = image && !imgError;

  return (
    <article className="border-b border-[#DAE8F4] pb-10 last:border-b-0">
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 text-md-custom font-medium leading-4 text-text-secondary">{donatedAmount}</p>
          <p className="mt-2 text-md-custom font-medium leading-6 text-text-secondary">{words}</p>
        </div>
      </div>
    </article>
  );
